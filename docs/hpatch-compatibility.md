...
`chiff_structured` totals 17131 bytes. The merged cover mode returns to 857
bytes, matching native hdiff, but it still pays the structured planning cost.
The current Hermes structured plan takes about 137s in a debug run on that pair.
The `native-coalesce` candidate in `node-hdiffpatch` reduces the same pair from
857 to 843 bytes by coalescing adjacent HDiffPatch-native covers with identical
old/new offset delta across small gaps. This is the strongest hpatch-compatible
lead so far, but production server usage should still remain opt-in,
serialized-size gated, and threshold-gated until a broader corpus proves it
non-regressive. With the default 4096-byte native-payload threshold, this pair
would skip structured planning entirely.
